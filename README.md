...
cargo run -p rusty-claude-cli -- --model GLM-5.1
...
Set your API credentials. The default provider is **GLM-5.1** (ZhipuAI):
...
Or authenticate via OAuth (Anthropic only) and let the CLI persist credentials locally:
...
| Feature | Status |
|---------|--------|
| Anthropic / OpenAI / xAI / GLM (ZhipuAI) provider flows + streaming | ✅ |
| OAuth login/logout | ✅ |
